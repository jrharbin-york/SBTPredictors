...
class HypervolumeWithGDRelativeQuality(IndicatorBasedDecisions):
    ...
    def relative_quality(self, predicted_test_metrics):
        ...
        if len(current_best_front_df) > 1:
            ...
            # sanity check: there should be as many GDs as there are points in the front!
            if len(other_gds) < len(current_best_front_df):
                print(f"len(other_gds) = {len(other_gds)}, len(current_best_front_df)={len(current_best_front_df)}")
                raise GDCalculationFailed

            rq = 1.0 - gd_for_hypothetical_point / np.max(other_gds)
            log.debug(f"predicted_test_metrics={predicted_test_metrics},gd_for_new_point={gd_for_hypothetical_point}, other_gds={other_gds}, rq={rq}")
        else:
            # if the front is too small (zero or one element) always give relative quality of 1 to new element
            rq = 1.0
            log.debug(f"Front is too small (zero or one element) - using relative_quality = 1.0")
